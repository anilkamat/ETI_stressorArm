...
def plot_3d_mesh(z,ModelName,m,config2,best_model_,savePath_results, conn_num, connections = '', type = ''):
    ...
    x = model_para_grid['reg_para']
    kernel = best_model_[ModelName+'kernel']
    reg_para = best_model_[ModelName+'reg_para']
    ...
    plt.ylabel('1')
    if m == 'all_C':
        ttl= str(type)+ModelName,',reg_para',reg_para,'kernel',kernel #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
        figName = str(type)+ModelName+'all_C'
    elif m =='single_C_add':
        ttl= str(type)+ModelName,'C_add_',str(connections),',reg_para',reg_para,',kernel',kernel    #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
        figName = str(type)+ModelName+'conn#'+str(conn_num)
    else:
        ttl= str(type)+ModelName,'set#',m,',reg_para',reg_para,',kernel',kernel  #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
...
def plot_CF_matrix(y_test_cv, y_test_pred_cv, QAD,connectivity_set, ModelName, set_number,savePath_results, labels, type):
    #print('y_test_cv, y_test_pred_cv', y_test_cv, y_test_pred_cv)
    #ConfusionMatrixDisplay.from_estimator(QDAclassifier, x_test, y_test) #, labels= ['MA','MA_reten']
    ...
    fileName = 'CF_'+str(type)+str(ModelName)+'_'+str(set_number)+'.xlsx'# +'C_'+str(QDAclassifier.C)+' gamma_'+str(QDAclassifier.gamma)
    xls_fileName = os.path.join(savePath_results, fileName)
    report.to_excel(xls_fileName)
    disp= ConfusionMatrixDisplay(confusion_matrix=cm,display_labels=labels)
    disp.plot()
    figName = 'CF_'+str(type)+str(ModelName)+'_'+str(set_number)# +'C_'+str(QDAclassifier.C)+' gamma_'+str(QDAclassifier.gamma)
    #print('figName :', figName)
    #print('QDAclassifier: ', QDAclassifier, QDAclassifier.C)
    #ttl = type+' reg_param:'+str(QAD.reg_param)+ str(ModelName)
    ttl = type+' reg_param:'+ str(ModelName)
    plt.title('\n'.join(wrap(ttl,60)))
    plt.savefig(os.path.join(savePath_results,figName))
    plt.close()
...
def analysis_tuned_model( data,n_folds, set_number,connectivity_set, \
    ...
    plt.ylabel('1')
    if m == 'all_C':
        ttl= str(type)+ModelName,',reg_para',reg_para #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
        figName = str(type)+ModelName+'all_C'
    elif m =='single_C_add':
        ttl= str(type)+ModelName,'C_add_',str(connections),',reg_para',reg_para    #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
        figName = str(type)+ModelName+'conn#'+str(conn_num)
    else:
        ttl= str(type)+ModelName,'set#',m,',reg_para',reg_para #,',best_test_acc',str('{:.3f}'.format(config2['acc_test'])
        figName = str(type)+ModelName+'set#'+str(m)
    plt.title(ttl)
    plt.savefig(os.path.join(savePath_results,figName))
    ...
    QDAclassifier = LinearDiscriminantAnalysis()
    ...
    for train_indx, test_indx in cv.split(X, Y):
        ...
        #acc_train = SVM(x_train,y_train, QDAclassifier)
        QDAclassifier.fit(x_train, y_train)
        
        y_pred_test = QDAclassifier.predict(x_test)
    ...
    df_metrics_write = pd.DataFrame({'reg_para':reg_para,'para_model_mcc': best_model_[ModelName+'test_mcc_CV'],\
    ...
    fname_mtx = 'Metrics'+'_set_'+str(set_number)+'_'+str(config['Days'])+str(config['trials'])+'.xlsx'  # storcs best model metrices.
    ...
    labels= ['SA_transfer','SA']
    plot_CF_matrix(y_test_cv, y_test_pred_cv, QDAclassifier,connectivity_set, ModelName, set_number,savePath_results, labels, type)
    plot_ROC(QDAclassifier, X,Y, n_folds,set_number,savePath_results, type)

def plot_ROC(QDAclassifier, X,y, n_folds,set_number,savePath_results, type): 
    cv = StratifiedKFold(n_splits=n_folds)
    #target_names = 'Mqn'
    ...
    for fold, (train_indx, test_indx) in enumerate(cv.split(X, y)):
        ...
        y_pred_test = QDAclassifier.predict(x_test)
        # ConfusionMatrixDisplay.from_estimator(QDAclassifier, x_test, y_test, labels= ['SA','MA'])
        #print('Confusion Mat: ', confusion_matrix(y_test,y_pred_test))
        #print(classification_report(y_test,y_pred_test))
        #plot_confusion_matrix(QDAclassifier, X_test, y_test)  
        acc_test = accuracy_score(y_test,y_pred_test)
    ...
    ax.set(
        ...
        title=f"Mean ROC curve with variability\n(Positive label '{target_names}'): {QDAclassifier}",
    )
    ax.axis("square")
