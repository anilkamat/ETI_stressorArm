...
from fNIRS_reten_grid_search_parameters_subtask_06062023 import grid_search
from fNIRS_reten_plot_mesh_grid_06062023 import plot_3d_mesh, analysis_tuned_model
import warnings
...
config={
  ...
  'trials':['T4_6'],  # for reten: 'trials':['T1_3']; for transfer 'trials':['T4_6'],
  'Days' : ['D4'],   # Both retention and transfer were performed on the same day after 8 weeks from training. 
    'model_para_grid':{'c' :np.arange(0.001,20,0.5),  # 'model_para_grid':{'c' :np.arange(0.001,10,0.5),# if change value, change in the grid search fxn as well
    'gamma':np.arange(0.001,20,0.5),   #'gamma':np.arange(0.001,10,0.5), 
    'kernel_' : ['rbf']}
}
...
for D in config['Days']:
    for T in config['trials']:
        ...
        #savePath_results =  os.path.join(r'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Stressor_arm\Results\SVM\stressorETI_vs_transfer_succ\SA_D3T1_3_vs_trans_D4T4_6_Succ')
