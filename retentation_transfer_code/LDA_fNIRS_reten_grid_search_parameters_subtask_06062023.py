...
import pandas as pd
import pickle
from LDA_fNIRS_reten_plot_mesh_grid_06062023 import plot_ROC
...
def grid_search( data, config_subtask, config, n_folds, set_number,savePath_results, Type = 'add_connectivity'):
    ...
    if Type == 'all_connectivity':
        ...
        para_model_[ModelName]=np.zeros(((1, len(model_para_grid['reg_para']))))
        para_model_with_mcc[ModelName]=np.zeros(((1, len(model_para_grid['reg_para']))))
        para_model_with_bas[ModelName]=np.zeros(((1, len(model_para_grid['reg_para']))))
        best_model_[ModelName+'reg_para'] = 0.0
    ...
    for kernel_ in model_para_grid['kernel_']:
        ...
        o = 0
        for reg_para in model_para_grid['reg_para']:
            LDA = []  #Resets classifier over loops
            if kernel_ == 'linear':
            ...
            if Type=='all_connectivity':
                ...
                for train_indx, test_indx in cv.split(X, Y):
                    ...
                    x_train,x_test = X.iloc[train_indx],X.iloc[test_indx]
                    y_train,y_test = Y.iloc[train_indx],Y.iloc[test_indx]
                    #acc_train = SVM(x_train,y_train, LDA)
                ...
                if (acc_test_CV_mean > best_model_[ModelName+'test_acc_CV']) :
                    #pickle.dump(LDA, open(ModelName+'.sav', 'wb'))     #save the best model so far
                    best_model_[ModelName+'reg_para'] = reg_para
                    ...
                    #plot_ROC(LDA, X,Y, n_folds,set_number,savePath_results, type = 'BaseLine')
                    # cm=confusion_matrix(y_test_cv,y_test_pred_cv)
                    #plot_CF_matrix(y_test_cv,y_test_pred_cv,LDA,connectivity_set,ModelName, \
                        #set_number, labels= ['SA','MA'], type = 'BaseLine')
                    if mcc_test_CV_mean > 0.40:
                        print('best test mcc:',mcc_test_CV_mean)
                        print(f'best parameters n: {n}, o: {o}, reg_para:{reg_para}')
            ...
            if Type=='add_connectivity':
                ...
                for train_indx, test_indx in cv.split(X, Y):
                    ...
                    mcc_test = matthews_corrcoef(y_test,y_pred_test)
                    bas_test = balanced_accuracy_score(y_test,y_pred_test)
                    #acc_test, mcc_test, bas_test = SVM_test(x_test,y_test, LDA)
                ...
                if (acc_test_CV_mean > best_model_[ModelName+'test_acc_CV']) :
                    ...
                    best_model_[ModelName+'test_mcc_CV'] =mcc_test_CV_mean
                    best_model_[ModelName+'test_bas_CV'] =bas_test_CV_mean
                    #plot_ROC(LDA, X,Y, n_folds,set_number,savePath_results, type = 'AddConnectivity')
                    #plot_CF_matrix(y_test_cv,y_test_pred_cv,LDA,connectivity_set,ModelName, \
                        #set_number, labels= ['SA','MA'], type = 'AddConnectivity')
                    if mcc_test_CV_mean > 0.40:
                        print('best test mcc:', mcc_test_CV_mean)
                        print(f'best parameters n: {n}, o: {o}, gamma_:{reg_para}')
                # store all the parameters and corrosponding accuracies
                para_model_[ModelName][n,o] = acc_test_CV_mean
                para_model_with_mcc[ModelName][n,o] = mcc_test_CV_mean
                para_model_with_bas[ModelName][n,o] = bas_test_CV_mean
