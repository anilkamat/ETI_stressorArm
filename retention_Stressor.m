...
addpath 'C:\Users\_Kamat_\Desktop\RPI\ResearchWork\Papers_\Effective_Connectivity\Stressor_arm\Datasets\ETI_SA_txts_noSS_ret'; % dataset
...
r = 1;
MyFolderInfo = dir('..\\Datasets\ETI_SA_txts_noSS_ret');
    ...
    for t = 1:nT                % loop over Trials
        ...
        for f = 1:nF
            ...
            if strcmp(file(5:11),subject)== 1
                Index = find(contains(stressor_performance(:,6),file(1:end-4)));
                class = cell2mat(stressor_performance(Index,7));
                fprintf('%s::: %s :::%d \n',file, subject, class);
                cF = cF+1;
                %fprintf('%s found \n',file)
